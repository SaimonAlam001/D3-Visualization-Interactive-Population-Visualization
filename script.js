d3.csv("data/countries_population.csv").then(function (data) {
  ...
  const verifiedEntities = ['Arab World', 'Caribbean small states', 'Central African Republic', 
  ...
  const countries = ["Top 10", ...countriesOnly.sort()];
  const entities = ["Top 10", "World", ...verifiedEntities.filter(e => e !== "World").sort()];
  ...
  updateCountryChart(latestYear, "Top 10");
  updateEntityChart(latestYear, "Top 10");
  ...
  function updateCountryChart(year, country) {
    ...
    const svg = d3.select("#countriesSection").append("svg")
      .attr("id", "countryChart").attr("width", 800).attr("height", 450);
    ...
    if (country === "Top 10") {
      ...
      svg.selectAll(".bar").data(filtered).enter().append("rect")
        .attr("x", d => x(d.Country)).attr("y", d => y(d.Population)).attr("width", x.bandwidth())
        .attr("height", d => height - margin.bottom - y(d.Population)).attr("fill", "steelblue")
  ...
  function updateEntityChart(year, entity) {
    ...
    const svg = d3.select("#entitiesSection").append("svg")
      .attr("id", "entityChart").attr("width", 800).attr("height", 450);
    const margin = { top: 50, right: 30, bottom: 100, left: 100 };
    const width = 800, height = 450;
    ...
    if (entity === "Top 10") {
      ...
      svg.selectAll(".bar").data(filtered).enter().append("rect")
        .attr("x", d => x(d.Country)).attr("y", d => y(d.Population)).attr("width", x.bandwidth())
        .attr("height", d => height - margin.bottom - y(d.Population)).attr("fill", "teal")
        .append("title").text(d => `${d.Country}: ${d.Population.toLocaleString()}`);
